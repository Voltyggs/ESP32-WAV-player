...
void handleVolumeButtons() {
  ...
  if (up == LOW) {
    ...
    if (volUpHeld && (now - lastAutoVolUp) >= AUTO_INTERVAL) {
      volume = min<uint8_t>(volume + 1, 255);
      lastAutoVolUp = now;
    }
...
void handlePlayButton() {
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    } else if (reading == HIGH) pressed = false;
  }
...
void loop() {
  ...
  if (nowMillis - lastSaveTime >= SAVE_INTERVAL) {
    saveSettings();
    lastSaveTime = nowMillis;
  }
